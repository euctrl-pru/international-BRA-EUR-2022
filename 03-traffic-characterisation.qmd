...
```{r, eval=FALSE}
# based on the updated data by BRAZIL - the graph has been fixed outside this 
# section
# the following code is deactivated 
# and the graph loaded below in the other code chunk.
# ACTION - ajust data files for corrected data / next report
g1 <- ds4 %>% ggplot() + 
...
knitr::include_graphics("./figures/corrected_traffic.png")
```

@fig-viz-annualvar shows the variation of air traffic across the study airports.
On average, the annual traffic at the Brazilian airports in 2020 decreased by about 47% and in 2021 traffic ranged about 32% lower than in 2019.
